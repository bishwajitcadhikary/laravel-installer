...
class LicenseBoxExternalAPI{
    ...
    public function __construct(){
        $this->product_id = '0A78C24F';
        $this->api_url = 'https://license.abndevs.net/';
        $this->api_key = '069E6DD071B53EF63614';
    ...
    public function verify_license($time_based_check = false, $license = false, $client = false){
        ...
        if($time_based_check && $this->verification_period > 0){
            ...
            $today = date('d-m-Y');
            if(empty($_SESSION["39a9173d8cc5bfe"])){
                $_SESSION["39a9173d8cc5bfe"] = '00-00-0000';
            }
            if($type == 1){
                $type_text = '1 day';
            }elseif($type == 3){
            ...
            if(strtotime($today) >= strtotime($_SESSION["39a9173d8cc5bfe"])){
                $get_data = $this->call_api(
                    'POST',
